...
function _to_composite(x::FieldsTower, y::FieldsTower, abs_disc::ZZRingElem)
  ...
  autK = Vector{NumFielHom{AbsSimpleNumField, AbsSimpleNumField}}(undef, length(x.generators_of_automorphisms)+ length(y.generators_of_automorphisms))
  ...
  embs = NumFielHom{AbsSimpleNumField, AbsSimpleNumField}[hom(Seemb, K, evaluate(mSeemb(gen(Seemb)).data, ev))]
