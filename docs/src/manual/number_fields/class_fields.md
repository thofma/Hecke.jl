...
of fractional ideals coprime to $m_0$ modulo the principal ideals generated by elements $a\in K^*$ such that

```math
v_p(a-1) \ge v_p(m_0) \mbox{ for all prime ideals } p \supseteq m_0.
```
and also

```math
a^{(v)}>0 \mbox{ for al places } v\in m_\infty.
```
...
is_totally_real(C::ClassField)
is_totally_complex(C::ClassField)
