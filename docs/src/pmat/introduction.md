...
```@docs
pseudo_matrix(m::AbstractAlgebra.MatElem{AbsSimpleNumFieldElem}, c::Vector{NfOrdIdl})
...
matrix(M::PMat)
base_ring(M::PMat)
pseudo_hnf(P::PMat{AbsSimpleNumFieldElem, NfOrdFracIdl})
pseudo_hnf_with_transform(P::PMat{AbsSimpleNumFieldElem, NfOrdFracIdl})
```
