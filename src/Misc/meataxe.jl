...
function closure(C::GenMat{fq_nmod}, G)
...
function spinning(C::GenMat{fq_nmod},G)
...
function _split(C::GenMat{fq_nmod},G)
# I am assuming that C is a Fp[G]-submodule

  equot=GenMat{fq_nmod}[]
  esub=GenMat{fq_nmod}[]
  pivotindex=Set{Int}()
  for i=1:rows(C)
    ind=1
...
function actsub(C::GenMat{fq_nmod},G)
...
function actquo(C::GenMat{fq_nmod},G)
