...
#
function cleanvect(M::GenMat{fq_nmod}, v::GenMat{fq_nmod})
...
function submatrix(M::GenMat{fq_nmod}, x::UnitRange{Int}, y::UnitRange{Int})
...
function closure(C::GenMat{fq_nmod}, G::Array{GenMat{fq_nmod},1})
...
function spinning(C::GenMat{fq_nmod},G::Array{GenMat{fq_nmod},1})
...
function clean_and_quotient(M::GenMat{fq_nmod},N::GenMat{fq_nmod}, pivotindex::Set{Int})
...
function _split(C::GenMat{fq_nmod},G::Array{GenMat{fq_nmod},1})
# I am assuming that C is a Fp[G]-submodule

  equot=Array{GenMat{fq_nmod},1}(length(G))
  esub=Array{GenMat{fq_nmod},1}(length(G))
  pivotindex=Set{Int}()
  for i=1:rows(C)
    ind=1
...
function actsub(C::GenMat{fq_nmod},G::Array{GenMat{fq_nmod},1})
...
function actquo(C::GenMat{fq_nmod},G::Array{GenMat{fq_nmod},1})
...
function meataxe(M::FqGModule)
  ...
  #
  G=deepcopy(H)
  Gt=GenMat{fq_nmod}[transpose(x) for x in M.G]
...
function composition_series(M::FqGModule)
  ...
  #
  list=Array{GenMat{fq_nmod},1}(length(sub_list)+length(quot_list))
...
function _irrsubs(M::FqGModule, N::FqGModule)
  ...
  if length(vects)==1
    return GenMat{fq_nmod}[closure(vects[1], N.G)]
  end
...
function submodules(M::FqGModule, index::Int; comp_factors=[])
  ...
  K=M.K
  if index==M.dim
    return GenMat{fq_nmod}[MatrixSpace(K,1,M.dim)()]
  end
  list=GenMat{fq_nmod}[]
  if index>= M.dim/2
    if comp_factors==[]
