...
#
function cleanvect(M::Generic.Mat{fq_nmod}, v::Generic.Mat{fq_nmod})
...
function submatrix(M::Generic.Mat{fq_nmod}, x::UnitRange{Int}, y::UnitRange{Int})
...
function closure(C::Generic.Mat{fq_nmod}, G::Array{Generic.Mat{fq_nmod},1})
...
function spinning(C::Generic.Mat{fq_nmod},G::Array{Generic.Mat{fq_nmod},1})
...
function clean_and_quotient(M::Generic.Mat{fq_nmod},N::Generic.Mat{fq_nmod}, pivotindex::Set{Int})
...
function _split(C::Generic.Mat{fq_nmod},G::Array{Generic.Mat{fq_nmod},1})
# I am assuming that C is a Fp[G]-submodule

  equot=Array{Generic.Mat{fq_nmod},1}(length(G))
  esub=Array{Generic.Mat{fq_nmod},1}(length(G))
  pivotindex=Set{Int}()
  for i=1:rows(C)
    ind=1
...
function actsub(C::Generic.Mat{fq_nmod},G::Array{Generic.Mat{fq_nmod},1})
...
function actquo(C::Generic.Mat{fq_nmod},G::Array{Generic.Mat{fq_nmod},1})
...
function _solve_unique(A::Generic.Mat{fq_nmod}, B::Generic.Mat{fq_nmod})
...
function meataxe(M::FqGModule)
  ...
  #
  G=deepcopy(H)
  Gt=Generic.Mat{fq_nmod}[transpose(x) for x in M.G]
...
function composition_series(M::FqGModule)
  ...
  #
  list=Array{Generic.Mat{fq_nmod},1}(length(sub_list)+length(quot_list))
...
function _irrsubs(M::FqGModule, N::FqGModule)
  ...
  if length(vects)==1
    return Generic.Mat{fq_nmod}[closure(vects[1], N.G)]
  end
  ...
  list=Array{Generic.Mat{fq_nmod},1}(length(candidate_comb))
...
function minimal_submodules(M::FqGModule, dim::Int=M.dim+1, lf=[])
  ...
  if M.isirreducible==true
    return Generic.Mat{fq_nmod}[]
  end

  list=Generic.Mat{fq_nmod}[]
  if isempty(lf)
...
function maximal_submodules(M::FqGModule, index::Int=M.dim, lf=[])
  ...
  maxlist=Array{Generic.Mat{fq_nmod},1}(length(minlist))
...
function submodules(M::FqGModule, index::Int; comp_factors=[])
  ...
  K=M.K
  if index==M.dim
    return Generic.Mat{fq_nmod}[MatrixSpace(K,1,M.dim)()]
  end
  list=Generic.Mat{fq_nmod}[]
  if index>= M.dim/2
    if comp_factors==[]
  ...
  # 
    ...
    list=Generic.Mat{fq_nmod}[transpose(nullspace(x)[2]) for x in dlist]
  end 
  return list
