...
"""
function rational_reconstruction(a::PolyRingElem{T}, b::PolyRingElem{T}; error_tolerant::Bool = false) where T
  return rational_reconstruction_subres(a, b, error_tolerant = error_tolerant)
end
function rational_reconstruction(a::QQPolyRingElem, b::QQPolyRingElem; error_tolerant::Bool = false)
  return rational_reconstruction_mod(a, b, error_tolerant = error_tolerant)
end


function rational_reconstruction(a::ZZRingElem, b::ZZRingElem; error_tolerant::Bool = false, kw...)
  return Nemo._rational_reconstruction(a, b; error_tolerant, kw...)
end
...
function berlekamp_massey_recon(a::Vector{T}; error_tolerant::Bool = false, parent = polynomial_ring(parent(a[1]), "x", cached = false)[1]) where T
  Rx = parent
  f = Rx(a)
  x = gen(Rx)
  xn= x^length(a)

  fl, n, d = rational_reconstruction(f, xn, error_tolerant = error_tolerant)
  if fl
    d = reverse(d)
...
function berlekamp_massey(L::Vector{QQFieldElem}; error_tolerant::Bool = false, parent = Globals.Qx)
  if error_tolerant
