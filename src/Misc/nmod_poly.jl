...
function rresx_sircana(f::PolyRingElem{T}, g::PolyRingElem{T}) where T <: ResElem{S} where S <: IntegerUnion
  ...
  if is_unit(leading_coefficient(g))
    q, r = divrem(u, g)
    @hassert :AbsNumFieldOrder{AbsSimpleNumField, AbsSimpleNumFieldElem} 1 res == r*f + (v+q*f)*g
    mul!(q, q, f)
    add!(v, v, q)
    return res, r, v
  else
    q, r = divrem(v, f)
    @hassert :AbsNumFieldOrder{AbsSimpleNumField, AbsSimpleNumFieldElem} 1 res == (u+q*g)*f + r*g
    mul!(q, q, g)
    add!(u, u, q)
    return res, u, r
  end
...
function fun_factor(f::T) where T <: Union{ZZModPolyRingElem, zzModPolyRingElem}
  ...
  @hassert :AbsNumFieldOrder{AbsSimpleNumField, AbsSimpleNumFieldElem} 1 f1 == u*g
  i = 1
  ...
  u0 = Rx(lift(Zy, u))
  g0 = Rx(lift(Zy, g))
  @hassert :AbsNumFieldOrder{AbsSimpleNumField, AbsSimpleNumFieldElem} 1 g0*u0 == f
  return u0, g0
end
