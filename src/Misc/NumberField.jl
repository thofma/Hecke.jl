...
function modular_init(K::AnticNumberField, p::Int)
  ...
  (den(K.pol) != 1) && error("Denominator of defining polynomial has to be 1")
  f = fmpz_poly()
  ccall((:fmpq_poly_get_numerator, :libflint), Void,
              (Ptr{fmpz_poly}, Ptr{fmpq_poly}), &f, &K.pol)
 
  # assert that p is a good prime
  # this is expensive
  @hassert :Nf 2 mod(disc(f), p) != 0

  UInt_p = UInt(p)

  # reduce mod p
  fmodp = nmod_poly(UInt_p)
  fmpz_poly_to_nmod_poly_raw!(fmodp, f)
  
  # factor
  # we cannot use factor() function since we don't want parent objects
  # copy code from Nemo/src/flint/nmod_poly.jl
  fac = Nemo.nmod_poly_factor(fmodp.mod_n)
  z = ccall((:nmod_poly_factor, :libflint), UInt,
          (Ptr{Nemo.nmod_poly_factor}, Ptr{nmod_poly}), &fac, &fmodp)
  ...
  for i in 1:fac.num
    ...
    ccall((:nmod_poly_factor_get_nmod_poly, :libflint), Void,
            (Ptr{nmod_poly}, Ptr{Nemo.nmod_poly_factor}, Int), &f, &fac, i-1)
  ...
  # I cannot construct the fields if the polynomials don't have a parent
  fake_nmodpoly = NmodPolyRing(ResidueRing(FlintZZ, p), :$, false)
  fields = Array{FqNmodFiniteField}(length(factors))
  for i in 1:length(factors)
    factors[i].parent = fake_nmodpoly
    fields[i] = FqNmodFiniteField(factors[i], :$)
  end

  # temporary variable
  amodp = nmod_poly(UInt_p)

  # construct the foward map K -> O/P_1 x ... x O/P_g
  reduce = function(a::nf_elem)
    # we don't do any sanity checks
    nf_elem_to_nmod_poly_den!(amodp, a)

    res = Array{fq_nmod}(length(factors))
