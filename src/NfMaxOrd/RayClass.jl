...
doc"""
***
    ray_class_group(m::NfMaxOrdIdl, A::Array{InfPlc,1}=[]) -> GrpAbFinGen, Map
...
***
  ...
  function conductor(S::GrpAbFinGen, mS::GrpAbFinGenMap, R::GrpAbFinGen, mR::MapRayClassGrp) -> NfMaxOrdIdl (, Array{InfPlc,1})
...
function conductor(S::GrpAbFinGen, mS::GrpAbFinGenMap, R::GrpAbFinGen, mR::MapRayClassGrp)
  ...
  if !isempty(mR.modulus_inf)
    cond_inf=[x for x in mR.modulus_inf]
    for i=1:length(mR.modulus_inf)
      candidate_inf=[x for x in cond_inf if x !=mR.modulus_inf[i]]
...
***
  ...
  function isconductor(R::GrpAbFinGen, mR::Map, m::NfMaxOrdIdl, infinite_primes::Array{InfPlc,1}=[]) -> Bool
...
function isconductor(S::GrpAbFinGen, mS::GrpAbFinGenMap, R::GrpAbFinGen, mR::MapRayClassGrp, m::NfMaxOrdIdl, infinite_places::Array{InfPlc,1}=InfPlc[])
  ...
  if !isempty(infinite_places)
    for i=1:length(infinite_places)
      candidate_inf=[x for x in infinite_places if x != infinite_places[i]]
...
doc"""
***
  function norm_group(f::Nemo.PolyElem, mR::Hecke.MapRayClassGrp, fin_modulus::NfMaxOrdIdl, inf_modulus::Array{InfPlc,1}=InfPlc[]) -> Hecke.GrpAbFinGen, Hecke.GrpAbFinGenMap
