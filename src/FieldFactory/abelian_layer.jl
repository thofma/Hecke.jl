...
function is_discriminant_square(IdG::GapObj)
...
function _construct_grp(IdH::GapObj, uncom::Int)
...
function _abelian_normal_extensions(F::FieldsTower, gtype::Array{Int, 1}, absbound::fmpz, IdCheck::GapObj, only_real::Bool, IdG::GapObj)
  K = F.field
  O = maximal_order(K) 
  n = prod(gtype)
...
function from_class_fields_to_fields(class_fields::Vector{Hecke.ClassField{Hecke.MapRayClassGrp, GrpAbFinGenMap}}, autos::Vector{NfToNfMor}, grp_to_be_checked::Dict{Int, GapObj}, target_group::GapObj)
...
function compute_fields(class_fields::Vector{Hecke.ClassField{Hecke.MapRayClassGrp, GrpAbFinGenMap}}, autos::Vector{NfToNfMor}, grp_to_be_checked::GapObj, right_grp)
...
function max_ab_norm_sub_containing(G::GapObj)
...
function create_sub(ss, iso, PermGAP, auts, K)
  ...
  lS1 = GapObj[GAP.Globals.Image(iso, lS[i]) for i in 1:length(lS)]
...
function compute_subfields(K::AnticNumberField, E, H, S)
  ...
  PermGAP = Vector{GapObj}(undef, length(Hperm))
