...
################################################################################
#
#  Core algorithm: GL2-equivalence test
#
################################################################################
...
function _is_isomorphic(C1::HypellCrv{T}, C2::HypellCrv{T}) where T
  ...
  # Reference: [Igu60], also implemented via Igusa-Clebsch invariants.
  if g == 2
    if igusa_clebsch_invariants(C1) != igusa_clebsch_invariants(C2)
      return false, dummy
    end
  end
