...
function subgroups(G::MultTableGroup; order::Int = 0,
  ...
  for i in 1:length(HH)
    res[i] = sub(G, HH[i]; complete = true)
  end
  return res
end

@doc raw"""
    sub(G::MultTableGroup, H::Vector{MultTableGroupElem}; complete::Bool = false)

Return the subgroup of $G$ generated by the elements of $H$, together with
its embedding into $G$.

If `complete` is `true`, $H$ must contain all elements of the subgroup,
without repetitions. In this case, the closure is not computed and the
ordering of $H$ is preserved.
"""
function sub(G::MultTableGroup, H::Vector{MultTableGroupElem}; complete::Bool = false)
  @req all(h -> parent(h) === G, H) "Elements must belong to the given group"
  if !complete
    H = closure(H, *, one(G))
  end
...
function intersect(mH::MultTableGroupHom, mK::MultTableGroupHom)
  ...
  return sub(codomain(mH), I; complete = true)
end
...
function center(G::MultTableGroup)
  if is_abelian(G)
    return sub(G, collect(G); complete = true)
  end
