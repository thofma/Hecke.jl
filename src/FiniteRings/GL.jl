function _presentation_sln(K, n)
  ...
  m = degree(K)
  a(b, i) = K isa Hecke.fqPolyRepField ? Int(Hecke.coeff(b, i)) : Int(lift(ZZ, Hecke.coeff(b, i)))
...
function EffectivePresentation(F::Oscar.FinField)
...
function _effective_presentation_of_glnq(G)
  n = degree(G)
  if n == 1
    return EffectivePresentation(G)
  end
  K = base_ring(G)
  H = Oscar.SL(n, K)
  A = _effective_presentation_of_slnq(H)
  #@info A
  C = EffectivePresentation(K)
  #@info C
  f = x -> G(x) # SL -> GL
...
function _effective_presentation_of_slnq_2(GM)
  ...
  if order(K) == 2 || characteristic(K) != 2
    return EffectivePresentation(GM)
  end
  @assert degree(GM) == 2
  q = order(K)
  FU, mFU = unit_group(K)
