...
function EffectivePresentation(A::EffectivePresentation, H, HtoA, AtoH)
  #@assert parent(AtoH(A.backward(rand(A.G)))) === H
  return EffectivePresentation(H, A.G, x -> 
                                begin
                                  #@show parent(x)
                                  #@show H
                                  #@show objectid(parent(x))
                                  #@show objectid(H)
                                  @assert parent(x) === H
                                 A.forward(HtoA(x)) 
                                end
...
function Oscar.preimage(A::EffectivePresentation, y)
  return A.backward(y)
end
...
function extension(A::EffectivePresentation, C::EffectivePresentation, B, f, fpreim, g, gpreim)
  ...
  #                       i    pi
  #
  # X, Y, Z are abstract groups, A, B are finitely presented groups.
  # I know all the maps. I compute a finitely presented group B and
  # the canonical map Y -> B making everything commute
  ...
  forward = function(b)
    ...
    #@show binHlifted
  #  w1 = C.forward(g(x))
  #  w11 = map_word(w1, gCinB; init = one(F))
  #  pi(xinHlifted) -> 
    #@show length(gensH_in_B)
...
function Base.show(io::IO, A::EffectivePresentation)
  println(io, "Interpretation of")
  println(io, A.A)
  println(io, "as the finitely presented group")
  print(io, A.G)
end

function _show_fp(G)
  #return "order: $(order(G)), $(ngens(G)) relators, $(length(relators(G))) relators"
