...
mutable struct ZLatAutoCtx{S, T, V, U}
  ...
  operate_cache::IdDict{T, Dict{Int, Int}} # memoizes _operate(point, A, V)
                       # for each generator matrix A encountered so far: since
                       # A is only ever built once and never mutated in place,
                       # caching on its identity turns the repeated orbit
                       # computations in _orbitlen/orbit/stab (which query the
                       # same (point, A) pairs over and over) into O(1)
                       # look-ups after the first query. A Dict (rather than a
                       # dense array of size 2*|V|+1) keeps the memory cost
                       # proportional to how many points were actually
                       # queried for each matrix, instead of |V|.
  ...
  function ZLatAutoCtx(G::Vector{ZZMatrix})
    ...
    z.operate_cache = IdDict{ZZMatrix, Dict{Int, Int}}()
