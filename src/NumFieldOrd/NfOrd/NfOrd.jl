...
number field.
"""
parent(O::AbsSimpleNumFieldOrder) = AbsNumFieldOrderSet(nf(O))
...
function _order(K::S, elt::Vector{T}; cached::Bool = true, check::Bool = true, extends = nothing) where {S <: Union{NumField{QQFieldElem}, AbstractAssociativeAlgebra{QQFieldElem}}, T}
  ...
  in_span_of_B = let dummy_vector = dummy_vector
    function(B, x::T)
    ...
    while true
        ...
        if !in_span_of_B(B, e_bj)
          if is_comm
            ...
            for k in 1:old_length
              t = bas[k]*e_bj
              !in_span_of_B(B, t) && push!(new_bas, t)
            end
          end
        end
      end
      ...
      if length(bas) >= n
        ...
        if full_rank
          ...
          mm = ZZ(m*denominator(B, copy = false))::ZZRingElem
          ...
          _hnf!_integral(B)
          k = findfirst(let B = B; k -> !is_zero_row(B, k) end, nrows(B) - n + 1:nrows(B))
...
The Dedekind different ideal of $R$, that is, $(R : T)$, where $T$ is the
trace dual of $T$.

For Gorenstein orders, this is also the inverse ideal of the codifferent.
"""
function different(R::AbsNumFieldOrder)
  # see https://stacks.math.columbia.edu/download/discriminant.pdf
  codiff = codifferent(R)
  if is_maximal_known_and_maximal(R)
    I = inv(codiff)
  else
    I = colon(denominator(codiff)*R, numerator(codiff))
  end
  @assert is_one(denominator(I))
  return numerator(I)
end
