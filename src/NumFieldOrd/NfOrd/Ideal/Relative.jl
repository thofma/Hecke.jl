...
function minimum(m::T, I::AbsNumFieldOrderIdeal{AbsSimpleNumField, AbsSimpleNumFieldElem}) where T <: Map{AbsSimpleNumField, AbsSimpleNumField}
  ...
  if !isone(gcd(minimum(I), index(order(I))))
    ...
    F = free_module(ZZ, degree(K))

    hsk = ModuleHomomorphism(free_module(ZZ, degree(k)), F, elem_type(F)[F(matrix(ZZ, 1, degree(K), coefficients(d*x))) for x = bk])
    hsK = ModuleHomomorphism(F, F, elem_type(F)[F(matrix(ZZ, 1, degree(K), coefficients(d*x))) for x = bK])
    sk = image(hsk)
    imhsK = image(hsK)
  ...
  #  hsk = hom(abelian_group[0 for i in 1:degree(k)], F, elem_type(F)[F(matrix(ZZ, 1, degree(K), coefficients(d*x))) for x = bk])
  #  hsK = hom(F, F, elem_type(F)[F(matrix(ZZ, 1, degree(K), coefficients(d*x))) for x = bK])
  #  sk = image(hsk)
  #  imhsK = image(hsK)
...
function intersect_nonindex(f::Map, P::AbsNumFieldOrderIdeal{AbsSimpleNumField, AbsSimpleNumFieldElem}, Zk::AbsSimpleNumFieldOrder = maximal_order(domain(f)))
  ...
  gp = factor(change_base_ring(base_ring(Fp), g; parent = Fp))
  hp = change_base_ring(base_ring(Fp), h; parent = Fp)
  Gp = gcd(Fp(K(P.gen_two)), change_base_ring(base_ring(Fp), G; parent = Fp))
  for (s, e) in gp
    if iszero(s(hp) % Gp)
...
function prime_decomposition_nonindex(f::Map, p::AbsNumFieldOrderIdeal{AbsSimpleNumField, AbsSimpleNumFieldElem}, ZK = maximal_order(codomain(f)))
  ...
  if fits(Int, minimum(p))
    ...
    Gp = factor(ppio(change_base_ring(base_ring(Fp), G; parent = Fp), Fp(f(p.gen_two.elem_in_nf)))[1])
    for (ke, e) in Gp
    ...
    Gp1 = factor(ppio(change_base_ring(base_ring(Fp1), G; parent = Fp1),
                      change_base_ring(base_ring(Fp1), Qx(f(K(p.gen_two))); parent = Fp1))[1])
    for (ke, e) in Gp1
...
function prime_decomposition_type_nonindex(f::Map, p::AbsNumFieldOrderIdeal{AbsSimpleNumField, AbsSimpleNumFieldElem}, ZK = maximal_order(codomain(f)))
  ...
  if fits(Int, minimum(p, copy = false))
    Fp = polynomial_ring(Native.GF(Int(minimum(p)), cached = false), cached = false)[1]
    Gp = factor_shape(gcd(Fp(f(K(p.gen_two))), change_base_ring(base_ring(Fp), G; parent = Fp)))
  else
    Fpp = polynomial_ring(Native.GF(minimum(p), cached = false), cached = false)[1]
    Gp = factor_shape(gcd(Fpp(f(K(p.gen_two))),
                          change_base_ring(base_ring(Fpp), G; parent = Fpp)))
  end
