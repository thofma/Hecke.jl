...
function ideal(O::GenOrd, a::RingElement)
  g = _as_coefficient_ring_elem(O, a)

  if g === nothing || is_zero(g)
    return GenOrdIdl(O, O(a))
  end

  return GenOrdIdl(O, g)
end
...
"""
has_princ_gen(A::GenOrdIdl) = isdefined(A, :princ_gen)

@doc raw"""
    has_2_elem(A::GenOrdIdl) -> Bool

Returns whether $A$ is generated by two elements.
"""
has_2_elem(A::GenOrdIdl) = isdefined(A, :gen_two)

# Generator fields are declared with abstract types: type assertions restore inference
...
_gens_normal(I::GenOrdIdl{S, T}) where {S, T} = I.gens_normal::elem_type(T)
...
function is_zero(I::GenOrdIdl)
  if I.is_zero == 0
    if isdefined(I, :princ_gen)
      I.is_zero = is_zero(_princ_gen(I)) ? 1 : 2
    elseif isdefined(I, :gen_one) && isdefined(I, :gen_two)
      I.is_zero = is_zero(_gen_one(I)) && is_zero(_gen_two(I)) ? 1 : 2
    else
...
function is_one(I::GenOrdIdl)
  has_norm(I) && return is_one(norm(I; copy = false))
  has_minimum(I) && return is_one(minimum(I; copy = false))

  # norm is faster to compute for principal and two-element-normal representations.
  # two-element representation will usually carry the norm,
  # so we check only for principal ideal, to decide what to compute
  has_princ_gen(I) && return is_one(norm(I; copy = false))
...
function has_2_elem_normal(A::GenOrdIdl)
  if isdefined(A, :gens_normal)
    return is_unit(_gen_one(A)) || !is_unit(_gens_normal(A))
  end
  if has_2_elem(A) && defines_2_normal(A)
    A.gens_normal = _gen_one(A)
    return true
  end
  return false
end
...
function assure_has_basis_matrix(A::GenOrdIdl)
  ...
  if has_2_elem_normal(A)
    ...
    V = vcat(scalar_matrix(base_ring(O), n, m), representation_matrix(mod(_gen_two(A), m)))
...
function assure_has_minimum(A::GenOrdIdl)
  ...
  if has_princ_gen(A)
    A.minimum = _minimum_principal(O, _princ_gen(A))
  elseif has_2_elem_normal(A)
    A.minimum = _minimum_2elem_normal(O, _gen_one(A), _gen_two(A))
...
function assure_has_norm(A::GenOrdIdl)
  ...
  if has_princ_gen(A)
    A.norm = _make_canonical_in(O, norm(_princ_gen(A)))
  elseif has_2_elem_normal(A)
    A.norm = _norm_2elem_normal(O, _gen_one(A), _gen_two(A))
  else
...
function Hecke.valuation(A::GenOrdIdl{S, T}, p::GenOrdIdl{S, T}) where {S, T}
  ...
  if has_2_elem_normal(p)
    beta = Hecke.numerator(inv(O.F(_gen_two(p))), O)
    newA = fractional_ideal(beta*A, _gen_one(p))
    while is_integral(newA)
      e += 1
      newA = fractional_ideal(numerator(beta*newA; copy = false), _gen_one(p))
    end
  else
