...
function zassenhaus(f::PolyElem{nf_elem}, P::NfOrdIdl; degset::Set{Int} = Set{Int}(collect(1:degree(f))))
  ...
  b = Hecke.landau_mignotte_bound(f)*upper_bound(sqrt(t2(lead(f))), fmpz)
  ...
  for d = 1:length(S)
    for s = Hecke.subsets(S, d)
      ...
      g = prod(s)
      g = change_base_ring(g, x->K(reco(zk(lead(f)*x), M, pM)))*(1//lead(f))
      if iszero(rem(f, g))
        push!(res, g)
        used = union(used, s)
...
function van_hoeij(f::PolyElem{nf_elem}, P::NfOrdIdl; prec_scale = 20)
  ...
  from = min(from, N)
  from = max(up_to, from)
  b = Hecke.cld_bound(f, vcat(0:up_to-1, from:N-1)) .* upper_bound(sqrt(t2(lead(f))), fmpz)
...
#        display(d)
        for v = values(d)
          #trivial test:
          if false # does not work with non-monics....
            a = prod(map(constant_coefficient, factor(vH.H)[v])) 
            if degree(P) == 1
              A = K(reco(lead(f)*order(P)(lift(a)), vH.Ml, vH.pMr))
            else
              A = K(reco(lead(f)*order(P)(preimage(mC, a)), vH.Ml, vH.pMr))
            end
            A = A//lead(f)
