...
@doc Markdown.doc"""
  function _subfield_basis(K, as)

> A basis for the subfield of K that is generated by the elements of as.
"""
...
function _subfield_primitive_element_from_basis(K::S, as::Vector{T}) where {
  ...
  i = findfirst(a -> degree(minpoly(a)) == d, elt)
  if i <= d
    return elt[i]
  end
...
@doc Markdown.doc"""
    subfield(K::S, elt::Vector{T}; isbasis::Bool = false) where {S <: Union{AnticNumberField, Hecke.NfRel}, T <: Union{nf_elem, Hecke.NfRelElem}}

> The number field k[elt] generated by the elements of elt over the base
> field k of K. If isbasis is true, it is assumed that elt holds a k-basis of
> a subfield.
"""
function subfield(K::S, elt::Vector{T}; isbasis::Bool = false) where {S <: Union{AnticNumberField, Hecke.NfRel}, T <: Union{nf_elem, Hecke.NfRelElem}}
  if length(elt) == 1
