...
@doc Markdown.doc"""
    fractional_ideal(O::AlgAssRelOrd, M::Generic.Mat, side::Symbol = :nothing)
      -> AlgAssRelOrdFracIdl

> Returns the fractional ideal of $O$ with basis matrix $M$.
> If the ideal is known to be a right/left/twosided ideal of $O$, `side` may be
> set to `:right`/`:left`/`:twosided` respectively.
"""
function fractional_ideal(O::AlgAssRelOrd{S, T}, M::Generic.Mat{S}, side::Symbol = :nothing) where { S, T }
  coeffs = deepcopy(basis_pmatrix(O, copy = false).coeffs)
  return fractional_ideal(O, PseudoMatrix(M, coeffs), side)
end

@doc Markdown.doc"""
    fractional_ideal(O::AlgAssRelOrd, x::AbsAlgAssElem) -> AlgAssRelOrdFracIdl

> Returns the twosided fractional principal ideal of $O$ generated by $x$.
"""
function fractional_ideal(O::AlgAssRelOrd{S, T}, x::AbsAlgAssElem{S}) where {S, T}
  @assert parent(x) == algebra(O)

  if iscommutative(O)
    a = fractional_ideal(O, x, :left)
    a.isright = 1
    return a
  end
  ...
  return fractional_ideal(O, M, :twosided, true)
end

@doc Markdown.doc"""
    fractional_ideal(O::AlgAssRelOrd, x::AbsAlgAssElem, action::Symbol)
      -> AlgAssRelOrdFracIdl

> Returns the fractional ideal $x \cdot O$, if `action == :left`, and $O \cdot x$,
> if `action == :right`.
"""
function fractional_ideal(O::AlgAssRelOrd{S, T}, x::AbsAlgAssElem{S}, action::Symbol) where {S, T}
  @assert parent(x) == algebra(O)
  M = representation_matrix(x, action)
  if action == :left
    a = fractional_ideal(O, M, :right)
  elseif action == :right
    a = fractional_ideal(O, M, :left)
  end
  if iszero(x)
    a.iszero = 1
  end
...
@doc Markdown.doc"""
    *(O::AlgAssRelOrd, x::AbsAlgAssElem) -> AlgAssRelOrdFracIdl
    *(x::AbsAlgAssElem, O::AlgAssRelOrd) -> AlgAssRelOrdFracIdl

> Returns the fractional ideal $O \cdot x$ or $x \cdot O$ respectively.
"""
*(O::AlgAssRelOrd{S, T}, x::AbsAlgAssElem{S}) where {S, T} = fractional_ideal(O, x, :right)
*(x::AbsAlgAssElem{S}, O::AlgAssRelOrd{S, T}) where {S, T} = fractional_ideal(O, x, :left)
...
# The result is a fractional ideal either of left_order(a) or of right_order(b)
# (depending on set_order).
function _mul_full_lattice_frac(a::Union{ AlgAssRelOrdIdl{S, T}, AlgAssRelOrdFracIdl{S, T} }, b::Union{ AlgAssRelOrdIdl{S, T}, AlgAssRelOrdFracIdl{S, T} }; set_order::Symbol = :left_a) where { S, T }
  A = algebra(order(a))
  PM = __mul_pseudo_bases(A, pseudo_basis(a, copy = false), pseudo_basis(b, copy = false))
