...
> set to `:right`/`:left`/`:twosided` respectively.
"""
function ideal_from_lattice_gens(A::AbsAlgAss{S}, O::AlgAssRelOrd{S, T}, v::Vector{ <: AbsAlgAssElem{S} }, side::Symbol = :nothing) where { S <: NumFieldElem, T }
...
function *(a::AlgAssRelOrdIdl{S, T}, x::AlgAssRelOrdElem{S, T}) where { S, T }
...
function *(x::AlgAssRelOrdElem{S, T}, a::AlgAssRelOrdIdl{S, T}) where { S, T }
...
colon(a::AlgAssRelOrdIdl{S, T}, b::AlgAssRelOrdIdl{S, T}) where { S, T } = divexact_right(a, b)
