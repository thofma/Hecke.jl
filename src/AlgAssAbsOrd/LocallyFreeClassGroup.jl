...
################################################################################
#
#  Torsion of K_0(O, K) (locally and global)
#
################################################################################

# Bley-Wilson, "Computations in relative algebraic K-groups"
function _p_part(O::AlgAssAbsOrd, f::AlgAssAbsOrdIdl, p::IntegerUnion)
  # assume f is two-sided
  B = basis_matrix_wrt(f, O)
  A = abelian_group(change_base_ring(ZZ, B))
  _, m = ppio(order(A), ZZ(p))
  P, PtoA = primary_part(A, m)
  return ideal_from_lattice_gens(algebra(O), O, [ elem_in_algebra(O(_eltseq(PtoA(x).coeff))) for x in gens(P)]) + f
end

function relative_k0_torsion(O::AlgAssAbsOrd)
  ps = prime_divisors(index(O, maximal_order(O)))
  return abelian_group(elementary_divisors(direct_product([relative_k0_torsion(O, p) for p in ps]...)[1]))
end

function relative_k0_torsion(O::AlgAssAbsOrd, p::IntegerUnion)
  A = algebra(O)
  OA = maximal_order(O)
  Z, ZtoA = center(A)
  OinZ = _as_order_of_smaller_algebra(ZtoA, O, OA)
  ...
  F = _p_part(O, _F, p)
  F.order = O
  _FinZ = _as_ideal_of_smaller_algebra(ZtoA, _F)
  OC = maximal_order(Z)
  FinZ = _p_part(OC, _FinZ, p)
  ...
  # nr is the reduced norm and OZ the maximal order in Z
  @vprintln :LocallyFreeClassGroup "Compute K_1"
  k1 = K1_order_mod_conductor(O, OA, F, _as_ideal_of_smaller_algebra(ZtoA, F))
  ...
  for x in k1
    ...
    s = preimage(mR, OCtoQ(OC(normred_over_center(elem_in_algebra(x, copy = false), ZtoA))))
    push!(k1_as_subgroup, s)
  end
