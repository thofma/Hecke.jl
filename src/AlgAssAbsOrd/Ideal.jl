...
@doc raw"""
    algebra(a::AlgAssAbsOrdIdl) -> AbstractAssociativeAlgebra
...
function ideal(O::AlgAssAbsOrd, x::AbstractAssociativeAlgebraElem)
  ...
  t1 = A()
  t2 = A()
  M = zero_matrix(QQ, dim(A)^2, dim(A))
  ...
  M = sub(_hnf_integral(M, :lowerleft), nrows(M) - degree(O) + 1:nrows(M), 1:ncols(M))
...
function ideal(O::AlgAssAbsOrd, x::AbstractAssociativeAlgebraElem, side::Symbol)
  ...
  if side == :left
    M = basis_matrix(O)*representation_matrix(x, :right)
  elseif side == :right
    M = basis_matrix(O)*representation_matrix(x, :left)
  else
...
$\mathbb Z$-lattice.
"""
function ideal_from_lattice_gens(A::AbstractAssociativeAlgebra{QQFieldElem}, v::Vector{ <: AbstractAssociativeAlgebraElem{QQFieldElem} })
  M = zero_matrix(QQ, max(length(v), dim(A)), dim(A))
  for i = 1:length(v)
...
@doc raw"""
     ideal_from_lattice_gens(A::AbstractAssociativeAlgebra, O::AlgAssAbsOrd,
...
function _zero_ideal(A::AbstractAssociativeAlgebra{QQFieldElem})
  a = ideal(A, zero_matrix(QQ, dim(A), dim(A)); M_in_hnf=true)
  a.iszero = 1
  return a
end

function _zero_ideal(A::AbstractAssociativeAlgebra{QQFieldElem}, O::AlgAssAbsOrd)
  a = _zero_ideal(A)
  a.order = O
...
function assure_has_basis_matrix_wrt(a::AlgAssAbsOrdIdl, O::AlgAssAbsOrd)
  ...
  M = _hnf_integral(basis_matrix(a, copy = false)*basis_matrix_inverse(O, copy = false), :lowerleft)
...
@doc raw"""
    basis_matrix(a::AlgAssAbsOrdIdl; copy::Bool = true) -> QQMatrix
...
@doc raw"""
    basis_mat_inv(a::AlgAssAbsOrdIdl; copy::Bool = true) -> QQMatrix
...
function integral_basis_matrix_wrt(a::AlgAssAbsOrdIdl, O::AlgAssAbsOrd; copy::Bool = true)
  ...
  @assert isone(denominator(M)) "Ideal is not integral"
  return numerator(M)
end
...
function sum(a::Vector{AlgAssAbsOrdIdl{S, T}}) where {S, T}
  ...
  mats = [basis_matrix(I) for I in a]
...
function *(a::AlgAssAbsOrdIdl{S, T}, b::AlgAssAbsOrdIdl{S, T}) where {S, T}
  ...
  if is_full_rank(a) && is_full_rank(b)
    el = denominator(b.basis_matrix) * a.eldiv_mul * denominator(a.basis_matrix) * b.eldiv_mul * denominator_of_structure_constant_table(A) * oned
    H = sub(_hnf_integral_modular_eldiv!(M, el), (d2 - d + 1):d2, 1:d)
    @hassert :AlgAssOrd 1 H == sub(_hnf_integral(M), (d2 - d + 1):d2, 1:d)
  else
    H = sub(_hnf_integral(M), (d2 - d + 1):d2, 1:d)
...
function intersect(a::AlgAssAbsOrdIdl{S, T}, b::AlgAssAbsOrdIdl{S, T}) where {S, T}
  ...
  M2 = hcat(zero_matrix(QQ, d, d), basis_matrix(b, copy = false))
  M = vcat(M1, M2)
  H = sub(_hnf_integral(M, :lowerleft), 1:d, 1:d)
...
function *(a::AlgAssAbsOrdIdl, x::Union{ Int, ZZRingElem, QQFieldElem })
  ...
  b = ideal(algebra(a), x*basis_matrix(a, copy = false); M_in_hnf=true)
  if isdefined(a, :left_order)
    b.left_order = left_order(a)
  end
  if isdefined(a, :right_order)
...
function *(x::T, a::AlgAssAbsOrdIdl) where {T <: Union{AssociativeAlgebraElem, GroupAlgebraElem, MatAlgebraElem}}
  ...
  M = basis_matrix(a, copy = false)* representation_matrix(x, :left)
  b = ideal(algebra(a), M)
  if isdefined(a, :right_order)
    b.right_order = right_order(a)
  end
...
@doc raw"""
    in(x::AbstractAssociativeAlgebraElem, a::AlgAssAbsOrdIdl) -> Bool
...
function _ring_of_multipliers_integral_ideal(I::AlgAssAbsOrdIdl, p::ZZRingElem = ZZRingElem(1))
  ...
  for i = 1:length(B)
    ...
    @hassert :AlgAssOrd 1 is_one(denominator(M))
    for s = 1:degree(O)
      for t = 1:degree(O)
        m[t + (i - 1)*degree(O), s] = numerator(M[s, t])
      end
    end
  end
...
function assure_has_norm(a::AlgAssAbsOrdIdl{S, T}, O::AlgAssAbsOrd{S, T}) where { S, T }
  ...
  a.norm[O] = abs(det(basis_matrix(a, copy = false))*det(basis_matrix_inverse(O, copy = false)))
  return nothing
end
...
function _islocally_free_left(O::AlgAssAbsOrd, I::AlgAssAbsOrdIdl, p::Union{Int, ZZRingElem})
  OpO, toOpO = StructureConstantAlgebra(O, p*O, p)
  J = radical(OpO)
  OJ, toOJ = quo(OpO, J)
  decOJ = decompose(OJ)
  ...
  pI = p*I
  IpI, toIpI = StructureConstantAlgebra(I, p*I, p)
...
generated by $p$.
"""
function pradical(O::AlgAssAbsOrd, p::IntegerUnion)
...
function denominator(a::AlgAssAbsOrdIdl{S, T}, O::AlgAssAbsOrd{S, T}; copy::Bool = true) where { S, T }
  ...
  M = basis_matrix(a, copy = false)*basis_matrix_inverse(O, copy = false)
  return denominator(M)
end
...
function contract(A::AlgAssAbsOrdIdl, O::AlgAssAbsOrd)
  ...
  return ideal(algebra(O), O, BI; side = :nothing)
end

intersect(O::AlgAssAbsOrd, A::AlgAssAbsOrdIdl) = contract(A, O)
intersect(A::AlgAssAbsOrdIdl, O::AlgAssAbsOrd) = contract(A, O)

# Returns I as an ideal with order O. Mostly for compatibility with AbsNumFieldOrderIdeal.
...
function _as_order_of_smaller_algebra(m::AbsAlgAssMor, O::AlgAssAbsOrd, OB::AlgAssAbsOrd)
  ...
  N = basis_matrix(O, copy = false)*basis_matrix_inverse(OB, copy = false)
  @assert denominator(N) == 1
  H = vcat(M, numerator(N))
  K = kernel(H, side = :left)
...
function _as_ideal_of_larger_algebra(m::AbsAlgAssMor, I::AlgAssAbsOrdIdl, O::AlgAssAbsOrd)
  ...
  M = zero_matrix(QQ, dim(A)*dim(B), dim(B))
  ...
  M = sub(_hnf_integral(M, :lowerleft), dim(B)*(dim(A) - 1) + 1:dim(A)*dim(B), 1:dim(B))
...
function _as_ideal_of_algebra(I::AbsNumFieldOrderIdeal, i::Int, O::AlgAssAbsOrd, one_ideals::Vector{Vector{T}}) where { T <: AbstractAssociativeAlgebraElem }
  A = algebra(O)
...
@attr ZZRingElem function minimum(P::AlgAssAbsOrdIdl)
  ...
  v = matrix(QQ, 1, nrows(M), coefficients(one(algebra(P))))
  m = denominator(v * M)
  return m
end
...
function maximal_integral_ideal_containing(I::AlgAssAbsOrdIdl, p::Union{ ZZRingElem, Int }, side::Symbol)
  ...
  t = zero_matrix(QQ, length(basis_c), degree(O))
  for i = 1:length(basis_c)
    b = toOP\(BtoOP(toC\(basis_c[i])))
    elem_to_mat_row!(t, i, elem_in_algebra(b, copy = false))
  end
  m = _hnf_integral(vcat(basis_matrix(P; copy = false), t), :lowerleft)
...
function primary_decomposition(I::AlgAssAbsOrdIdl, O::AlgAssAbsOrd = left_order(I))
  ...
  M = numerator(basis_matrix(I) * basis_matrix_inverse(O; copy = false))
