...
"""
*(O::AlgAssAbsOrd{S, T}, x::T) where {S, T} = ideal(O, x, :left)
*(x::T, O::AlgAssAbsOrd{S, T}) where {S, T} = ideal(O, x, :right)
...
*(x::Union{ Int, fmpz }, O::AlgAssAbsOrd) = ideal(O, O(x), :right)
...
> $\mathbb Z$-lattice.
"""
function ideal_from_lattice_gens(A::AbsAlgAss{fmpq}, v::Vector{ <: AbsAlgAssElem{fmpq} })
...
> set to `:right`/`:left`/`:twosided` respectively.
"""
function ideal_from_lattice_gens(A::S, O::AlgAssAbsOrd{S, T}, v::Vector{T}, side::Symbol = :nothing) where { S <: AbsAlgAss{fmpq}, T <: AbsAlgAssElem{fmpq} }
...
function _zero_ideal(A::AbsAlgAss{fmpq})
  a = ideal(A, FakeFmpqMat(zero_matrix(FlintZZ, dim(A), dim(A)), fmpz(1)), true)
  a.iszero = 1
  return a
end

function _zero_ideal(A::AbsAlgAss{fmpq}, O::AlgAssAbsOrd)
  a = _zero_ideal(A)
  a.order = O
...
@doc Markdown.doc"""
    ...
    *(a::AlgAssAbsOrdIdl, x::fmpz) -> AlgAssAbsOrdIdl
    *(x::fmpz, a::AlgAssAbsOrdIdl) -> AlgAssAbsOrdIdl
    *(a::AlgAssAbsOrdIdl, x::fmpq) -> AlgAssAbsOrdIdl
    *(x::fmpq, a::AlgAssAbsOrdIdl) -> AlgAssAbsOrdIdl
...
"""
function *(a::AlgAssAbsOrdIdl, x::Union{ Int, fmpz, fmpq })
  if iszero(x)
...
function _ring_of_multipliers_integral_ideal(I::AlgAssAbsOrdIdl, p::fmpz = fmpz(1))
  O = order(I)
...
function _colon_raw(a::AlgAssAbsOrdIdl{S, T}, b::AlgAssAbsOrdIdl{S, T}, side::Symbol) where { S, T }
  ...
  d = dim(A)
  bb = basis(b, copy = false)
  B = fmpq_mat(basis_mat_inv(a, copy = false))
...
@doc Markdown.doc"""
    norm(a::AlgAssAbsOrdIdl; copy::Bool = true) -> fmpq
...
@doc Markdown.doc"""
    normred(a::AlgAssAbsOrdIdl, O::AlgAssAbsOrd; copy::Bool = true) -> fmpq
...
@doc Markdown.doc"""
    normred(a::AlgAssAbsOrdIdl; copy::Bool = true) -> fmpq
...
@doc Markdown.doc"""
    locally_free_basis(a::AlgAssAbsOrdIdl, p::Union{ Int, fmpz })
      -> AlgAssAbsOrdElem
...
@doc Markdown.doc"""
    locally_free_basis(O::AlgAssAbsOrd, a::AlgAssAbsOrdIdl,
                       p::Union{ Int, fmpz }) -> AlgAssAbsOrdElem
...
@doc Markdown.doc"""
    denominator(a::AlgAssAbsOrdIdl, O::AlgAssAbsOrd) -> fmpz
...
@doc Markdown.doc"""
    denominator(a::AlgAssAbsOrdIdl) -> fmpz
...
function _as_ideal_of_smaller_algebra(m::AbsAlgAssMor, I::AlgAssAbsOrdIdl)
  ...
  M = FakeFmpqMat(M)
  M1 = hcat(M, M)
  M2 = hcat(FakeFmpqMat(zero_matrix(FlintZZ, dim(B), dim(B)), fmpz(1)), basis_matrix(I, copy = false))
