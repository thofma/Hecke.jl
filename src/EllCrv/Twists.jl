...
#Adapted from Magma code
  K = base_field(E)
  if isodd(degree(K))
    return EllCrv{T}[elliptic_curve(K, [0, 0, 1, 0, 0]), elliptic_curve(K, [0, 0, 1, 1, 0]), elliptic_curve(K, [0, 0, 1, 1, 1]) ]
  end
  ...
  #First three curves
  part_1 = EllCrv{T}[elliptic_curve(K, [0, 0, 1, 0, 0]), elliptic_curve(K, [0, 0, 1, c, 0]), elliptic_curve(K, [0, 0, 1, 0, c]) ]
  #The other four
  part_2 = EllCrv{T}[elliptic_curve(K, [0, 0, a, 0, a^2*b]) for (a, b) in [[u, 0], [inv(u), 0], [u, c], [inv(u), c]]]
...
#Adapted from Magma code
    ...
    if isodd(d)
      return EllCrv{T}[elliptic_curve(K, [1,0]), elliptic_curve(K, [-1, 0]), elliptic_curve(K, [-1, c]), elliptic_curve(K, [-1, -c]) ];
    end
    u = gen(K);
    #First four curves
    part_1 = EllCrv{T}[elliptic_curve(K, [-u^i,0]) for i in (0:3)]
    part_2 = EllCrv{T}[elliptic_curve(K, [-1,c]), elliptic_curve(K, [-u^2,(u^3)*c])]
    return vcat(part_1, part_2)
...
#Adapted from Magma code
   ...
   elseif j == 0
      ...
      c = -c6//864
      n = gcd(6, order(K)-1)
      return EllCrv{T}[ elliptic_curve(K, [0,c*a^i]) for i in (0:n-1) ]
   elseif j == 1728
      a = gen(K)
      c4, c6 = c_invars(E)
      c = -c4//48;
      n = gcd(4, order(K)-1)
      return EllCrv{T}[ elliptic_curve(K, [c*a^i,0]) for i in (0:n-1)]
   end
