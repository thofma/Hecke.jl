...
function locally_free_class_group(O::AlgAssAbsOrd, cond::Symbol = :default)
  A = algebra(O)
  Z, ZtoA = center(A)
  OA = maximal_order(A)
  if cond == :left
...
function K1_order_mod_conductor(O::AlgAssAbsOrd, F::AlgAssAbsOrdIdl, FinZ::AlgAssAbsOrdIdl)
  ...
  k1 = Vector{elem_type(O)}()
  oneO = one(O)
  t = [ oneO for i = 1:length(moduli) ]
  for i = 1:length(primary_ideals)
    ...
    if pF != qF
      H, HtoO = _1_plus_p_mod_1_plus_q(pF, qF)
      for i = 1:ngens(H)
        push!(k1_O, HtoO(H[i]))
      end
    end
