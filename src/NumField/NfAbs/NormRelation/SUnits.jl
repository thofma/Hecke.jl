...
function __sunit_group_fac_elem_quo_via_brauer(N::NormRelation, O, S::Vector{AbsNumFieldOrderIdeal{AbsSimpleNumField, AbsSimpleNumFieldElem}}, n::Int, invariant::Bool = false, compact::Int = 0; saturate_units::Bool = false, GRH::Bool = true)
  ...
  @assert ngens(Q) == 1
  m = order(Q)
  units, res_group = if !isone(m)
    us = Vector{FacElem{AbsSimpleNumFieldElem, AbsSimpleNumField}}(undef, length(unitsmodtorsion)+1)
    us[1] = FacElem(elem_in_nf(mT(mQ\Q[1])))
    for i = 2:length(us)
      us[i] = unitsmodtorsion[i-1]
    end
    us, abelian_group(append!(ZZRingElem[m], [ZZRingElem(n) for i in 1:(length(sunitsmodunits) + length(unitsmodtorsion))]))
  else
    unitsmodtorsion, abelian_group([ZZRingElem(n) for i in 1:(length(sunitsmodunits) + length(unitsmodtorsion))])
  end

  function exp(a::FinGenAbGroupElem)
    @assert parent(a) == res_group
    y = prod(units[i]^a[i] for i = 1:length(units))
    if !isempty(sunitsmodunits)
      yy = prod(sunitsmodunits[i]^a[length(units) + i] for i in 1:length(sunitsmodunits))
      mul!(y, y, yy)
    end
