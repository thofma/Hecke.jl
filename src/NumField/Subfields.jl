...
function _subfield_basis(K::S, as::Vector{T}) where {
    S <: Union{AbsSimpleNumField, Hecke.NfRel},
    T <: Union{AbsSimpleNumFieldElem, Hecke.NfRelElem}
   }
  if isempty(as)
...
#can be disabled by setting `simplify = false`.
#"""
function fixed_field(K::AbsSimpleNumField, A::Vector{NfToNfMor}; simplify::Bool = true)
...
function fixed_field1(K::AbsSimpleNumField, auts::Vector{NfToNfMor})
...
function fixed_field(K::AbsSimpleNumField, auts::Vector{NfToNfMor}, ::Type{NfRel{AbsSimpleNumFieldElem}}; simplify_subfield::Bool = true)
  ...
  def_eq = map_coefficients(x -> has_preimage_with_preimage(mF, x)[2], p)
