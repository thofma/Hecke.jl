...
def find_previous_version(version: str) -> str:
    ...
    if major != 0:
        error("unexpected Hecke version, not starting with '0.'")
    if patchlevel != 0:
        patchlevel -= 1
...
topics = [
    ...
    ["topic: number fields",        "Number fields"],
    ["topic: function fields",      "Function fields"],
    ["topic: algebras",             "Noncommutative algebras"],
...
def pr_to_md(pr: Dict[str, Any]) -> str:
    ...
    k = pr["number"]
    title = pr["title"]
    # we do some filtering, since often title = commit message = starting with doc:, :fix or :feat
    l = len(title)
    for s in ["doc:", "fix:", "feat:"]:
        title = title.removeprefix(s)
    if l > len(title):
        # we removed a prefix
        # fix white spaces and capitalization
        title = title.strip()
        title = title[0].upper() + title[1:]
    return f"- [#{k}](https://github.com/thofma/Hecke.jl/pull/{k}) {title}\n"
...
def changes_overview(
    ...
    release_url = f"https://github.com/thofma/Hecke/releases/tag/v{new_version}"
...
"""
        ...
        #for pr in prs:
        #    print(pr)
        #    print(has_label(pr, "topic: number fields"))
        countedPRs = 0
        for priorityobject in topics:
            matches = [
                pr for pr in prs_with_use_title if has_label(pr, priorityobject[0])
            ]
            print(priorityobject[0] == "topic: number fields")
            print(len(prs_with_use_title))
...
def main(new_version: str) -> None:
    ...
    if major != 0:
        error("unexpected Hecke version, not starting with '0.'")
    if patchlevel == 0:
        # Hecke release which changes just the minor version
