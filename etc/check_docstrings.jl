...
#
# It is run as part of the Hecke CI tests, but it can also be run manually
#
# It can be run from the command line with:
# > julia --project=. -e 'using Hecke; include("etc/check_docstrings.jl")'
#
# it is also possible to test more packages at once (as long as Main.X exists)
# > julia --project=. -e 'using Hecke; include("etc/check_docstrings.jl")' Nemo Hecke
#
...
function get_broken_docstrings(m::Module)
  ...
  while !isempty(allpairs)
    ...
    elseif !isnothing(memberobj)
      docs = applicable(Base.Docs.doc, memberobj) ? Base.Docs.doc(memberobj) : nothing
...
function (@main)(args)
  modnames = length(args) > 0 ? args : ["Hecke"]
