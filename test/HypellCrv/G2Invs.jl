@testset "G2 Invariants" begin
    ...
    cl_invs, ws = @inferred clebsch_invariants(C)
    ig_cl_invs, ws = @inferred igusa_clebsch_invariants(C)
    ig_invs, ws = @inferred igusa_invariants(C)
    g2_invs = @inferred g2_invariants(C)

    @test cl_invs == clebsch_invariants(f1, h1)[1]
    ...
    @test weighted_equality(clebsch_invariants(C)[1],[ F(9), F(18), F(26), F(24)] ,[2,4,6,10])
    @test weighted_equality(igusa_clebsch_invariants(C)[1],[ F(30), F(21), F(29), F(9) ] ,[2,4,6,10])
    @test weighted_equality(igusa_invariants(C)[1], [ F(13), F(18), F(20), F(21), F(16) ]  ,[2,4,6,8,10])
    @test g2_invariants(C) == [ F(23), F(25), F(17) ]


    @test weighted_equality(igusa_clebsch_from_igusa(igusa_invariants(C)[1]), igusa_clebsch_invariants(C)[1], [2,4,6,10])
    @test weighted_equality(clebsch_from_igusa_clebsch(igusa_clebsch_invariants(C)[1]), clebsch_invariants(C)[1], [2,4,6,10])
    @test weighted_equality(igusa_from_g2(g2_invariants(C)), igusa_invariants(C)[1], [2,4,6,8,10])
    ...
    @test weighted_equality(ig_invs,[ F(0), F(0), F(0), F(1), F(a^12)], ws)
    @test g2_invs == [ F(0), F(0), F(a^9) ]
    @test weighted_equality(igusa_from_g2(g2_invs), ig_invs, ws)
    ...
    f1 = x^6+3*x^5-4*x^4+3*x^2+2*x-1
    C = hyperelliptic_curve(f1)
    @test weighted_equality(clebsch_invariants(C)[1],[ F(0), F(2), F(2), F(0)] ,[2,4,6,10])
    @test weighted_equality(igusa_clebsch_invariants(C)[1],[ F(0), F(4), F(1), F(3) ] ,[2,4,6,10])
    @test weighted_equality(igusa_invariants(C)[1],[ F(0), F(2), F(3), F(6), F(3)] ,[2,4,6,8,10])
