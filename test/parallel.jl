...
for (testname, (resp,)) in results
  ...
  elseif isa(resp, Test.TestSetException)
    ...
    for i in 1:resp.pass
      Test.record(fake, VERSION < 1.7.0-DEV.1196 ? Test.Pass(:test, nothing, nothing, nothing) : Test.Pass(:test, nothing, nothing, nothing, LineNumberNode(@__LINE__, @__FILE__)))
    end
