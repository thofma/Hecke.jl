@testset "FieldFactory" begin
  ...
  @time begin
    @testset "Quadratic Fields" begin
      lq = fields(2, 1, ZZRingElem(10)^4)
      @test length(lq) == 6086

      lqreal = fields(2, 1, ZZRingElem(10)^4, only_real = true)
  ...
  @time begin
    @testset "Biquadratic Fields" begin
      l = fields(4, 2, ZZRingElem(10)^6)
      @test length(l) ==  1014

      l1 = fields(4, 2, ZZRingElem(10)^6, only_real = true)
      @test length(l1) == 196
  ...
  @time begin
    @testset "Abelian fields" begin
      l = fields(6, 2, ZZRingElem(10)^6)
      @test length(l) ==  10

      l1 = fields(9, 2, ZZRingElem(10)^13)
      @test length(l1) == 4
    end
  end
  ...
  @time begin
    @testset "Direct product decomposition" begin
      @time l_direct_product = fields(12, 4, ZZRingElem(10)^13)
      @test length(l_direct_product) == 25

      @time l_without = fields(12, 4, ZZRingElem(10)^13, using_direct_product = false)
      @test length(l_without) == 25

      d1 = length(fields(24, 13, ZZRingElem(10)^24))
      d2 = length(fields(24, 13, ZZRingElem(10)^24, using_direct_product = false))
      @test d1 == d2

      @time l_direct_product = fields(18, 3, ZZRingElem(10)^18)
      @time l_without = fields(18, 3, ZZRingElem(10)^18, using_direct_product = false)
  ...
  @time begin
    @testset "Split extension" begin
      ...
      l1 = fields(42, 5, l, ZZRingElem(10)^90)
      @test length(l1) == 1
  ...
  @time begin
    @testset "Obstructions: degree 2" begin
      ...
      lQ8 = fields(8, 4, ZZRingElem(10)^12)
      @test length(lQ8) == 2

      lQ8_2 = fields(8, 4, forQ8, ZZRingElem(10)^12)
      @test length(lQ8_2) == 2

      lQ8real = fields(8, 4, forQ8, ZZRingElem(10)^12, only_real = true)
  ...
  @time begin
    @testset "Some examples" begin
      ...
      @time @test length(fields(64, 6, l, ZZRingElem(80)^64)) == 1
