...
@testset "Automorphisms" begin
  ...
  @testset "Subgroup generated by complex conjugation" begin
    f = x^12 + 124*x^10 + 4433*x^8 + 56544*x^6 + 276768*x^4 +
        492032*x^2 + 246016
    K, a = number_field(f)
    G, mG = automorphism_group(K)
    emb = complex_embeddings(K)[1]
    sigma = only(g for g in G if overlaps(conj(emb(a)), emb(mG(g)(a))))
    D, mD = sub(G, [sigma])
    @test order(D) == 2
    @test Set(mD(d) for d in D) == Set([one(G), sigma])
    @test mD(one(D)) == one(G)
    @test mG(mD(preimage(mD, sigma)))(a) == -a
  end
